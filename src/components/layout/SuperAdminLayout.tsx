...
const menuItems = [
  { to: "/superadmin/dashboard", icon: <LayoutDashboard size={18} />, label: "Dashboard Global" },
  { to: "/superadmin/etablissements", icon: <School size={18} />, label: "Établissements" },
  { to: "/superadmin/administrateurs", icon: <UserCog size={18} />, label: "Administrateurs" },
  { to: "/superadmin/utilisateurs", icon: <Users size={18} />, label: "Utilisateurs" },
  { to: "/superadmin/licences", icon: <CreditCard size={18} />, label: "Licences & Abonnements" },
  { to: "/superadmin/analytics", icon: <BarChart3 size={18} />, label: "Analytics & Rapports" },
  { to: "/superadmin/notifications", icon: <Bell size={18} />, label: "Notifications Système", badge: 2 },
  { to: "/superadmin/securite", icon: <Shield size={18} />, label: "Sécurité & Accès" },
  { to: "/superadmin/logs", icon: <FileText size={18} />, label: "Logs & Audit" },
  { to: "/superadmin/parametres", icon: <Settings size={18} />, label: "Paramètres Plateforme" },
  { to: "/superadmin/support", icon: <HeadphonesIcon size={18} />, label: "Support & Incidents", badge: 5 },
];

const superAdminUser = {
  name: "Super Admin",
  email: "admin@argon.dev",
        ...
        <div className="h-16 px-6 flex items-center border-b border-border">
          <Link to="/superadmin/dashboard" className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center shadow-lg">
              <Command className="text-primary-foreground" size={18} />
            </div>
            <div>
        ...
        <div className="p-3 border-t border-border">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
            <Avatar className="h-9 w-9">
              <AvatarImage src={superAdminUser.avatar} />
              <AvatarFallback>SA</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">{superAdminUser.name}</p>
              <p className="text-xs text-muted-foreground truncate">{superAdminUser.email}</p>
            </div>
            ...
            >
              ...
              <div className="p-3 border-t border-border">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{superAdminUser.name}</p>
                    <p className="text-xs text-muted-foreground truncate">{superAdminUser.role}</p>
                  </div>
                </div>
              </div>
            </motion.aside>
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-2 px-2 ml-2">
                    ...
                    <div className="hidden md:block text-left">
                      <p className="text-sm font-medium">{superAdminUser.name}</p>
                    </div>
